...
public class SedPreferencesTest {
    ...
    public void testAddMultipleSegments() throws Exception {
        ...
        // Rename second segment target
        sed.getSegment(1).getTarget().setName(new TextParam("my segment"));
        
        // Verify
        model.refresh();

        assertEquals("target1", model.getSegmentModel(sed.getSegment(0)).getSuffix());
        assertEquals("my segment", model.getSegmentModel(sed.getSegment(1)).getSuffix());
    }
