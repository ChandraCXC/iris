...
public class VizierImporter {
    ...
    public static Collection<Segment> getSedFromName(String targetName, Double searchRadius, String endpoint) throws SegmentImporterException {
        try {
            ...
            StarTable table = vob.makeStarTable(new URLDataSource(cdsUrl), true, StoragePolicy.ADAPTIVE);
            ...
            for (long i = 0; i < table.getRowCount(); i++) {
                ...
                String recno = (String) table.getCell(i, 2);
                if (!segMap.containsKey(recno)) {
                    ...
                    s.createTarget().createName().setValue(targetName + " " + recno);
                    ...
                    s.createCuration().createPublisher().setValue("Vizier - CDS - table: " + recno);
                }
                Segment s = segMap.get(recno);
                Point p = new Point();
