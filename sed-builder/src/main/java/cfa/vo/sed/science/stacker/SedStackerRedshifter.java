...
public class SedStackerRedshifter {
            ...
            throws Exception 
    {
        ...
        for (int i = 0; i < stack.getSeds().size(); i++) {
            ...
            segment.setYerr((double[]) stack.getSed(i).getSegment(0)
        ...
        for (SegmentPayload segment : response.getSegments()) {
            ...
            stack.getSed(c).getSegment(0).setDataValues(segment.getYerr(),
                    SEDMultiSegmentSpectrum.E_UTYPE);

            // If the SED's original redshift is known, reset the latest
            // redshift
            ...
            if (Integer.parseInt(stack.getSed(c).getAttachment(SedStackerAttachments.REDSHIFT_CONF_HASH).toString()) != zconfig
                    .hashCode()) {
        ...
        if (response.getExcludeds() != null && response.getExcludeds().size() > 0) {
            NarrowOptionPane.showMessageDialog(null, "SEDs " + response.getExcludeds()
                    + " were not redshifted because they do not have redshifts.", "Unshifted SEDs",
    ...
    private void convertUnits(SedStack stack, List<String> xUnits, List<String> yUnits) throws SedException,
            UnitsException {
        for (int i = 0; i < stack.getSeds().size(); i++) {
