...
public class SedStackerStacker {
     ...
     *  2. Confirm suffient memory to perform operation.
    ...
    protected SedStackerStackPayload translateResponse(Response rspns) throws Exception {
        return (SedStackerStackPayload) SAMPFactory.get(rspns.getResult(), 
            ...
            throws SedException, UnitsException 
    {
        for (int i = 0; i < stack.getSeds().size(); i++) {
            ...
            stack.getSeds().get(i).getSegment(0).setFluxAxisValues(nsed.getSegment(0)
                    .getFluxAxisValues());
            stack.getSeds().get(i).getSegment(0).setSpectralAxisValues(nsed.getSegment(0)
                    .getSpectralAxisValues());
            stack.getSeds().get(i).getSegment(0).setDataValues((double[]) nsed.getSegment(0)
                    .getDataValues(UTYPE.FLUX_STAT_ERROR), UTYPE.FLUX_STAT_ERROR);
        }
    }
