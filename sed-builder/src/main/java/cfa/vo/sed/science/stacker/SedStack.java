...
public class SedStack implements Cloneable {
    ...
    private String name;
    private List<ExtSed> seds = ObservableCollections.observableList(new ArrayList<ExtSed>());
    private List<ExtSed> origSeds = ObservableCollections.observableList(new ArrayList<ExtSed>());
    ...
    // propertyChangeSupport.firePropertyChange(PROP_SEDBUILDERSTACK,
    // oldSedBuilderStack, sedBuilderStack);
    // }
    ...
    public List<String> getSpectralUnits() {
        try {
            ...
            Logger.getLogger(SedStack.class.getName()).log(Level.SEVERE, null,
                    ex);
        }
        // TODO: What will change if we return an empty list instead?
        return null;
    }
    ...
    public void add(ExtSed sed) throws SedNoDataException, SedException,
            UnitsException 
    {
        ExtSed nsed = ExtSed.flatten(sed, sed.getSegment(0)
                .getSpectralAxisUnits(), sed.getSegment(0).getFluxAxisUnits());
        nsed.setId(sed.getId());
        nsed.addAttachment(SedStackerAttachments.ORIG_REDSHIFT,
                sed.getAttachment(SedStackerAttachments.ORIG_REDSHIFT));
        nsed.addAttachment(SedStackerAttachments.REDSHIFT,
                sed.getAttachment(SedStackerAttachments.REDSHIFT));
        nsed.addAttachment(SedStackerAttachments.NORM_CONSTANT,
                sed.getAttachment(SedStackerAttachments.NORM_CONSTANT));
        nsed.addAttachment(SedStackerAttachments.NORM_CONF_HASH, 1);
        nsed.addAttachment(SedStackerAttachments.REDSHIFT_CONF_HASH, 1);

        seds.add(nsed);
        origSeds.add(sed);
        // Segment seg = ExtSed.flatten(sed,
        // sed.getSegment(0).getSpectralAxisUnits(),
        // sed.getSegment(0).getFluxAxisUnits()).getSegment(0);
        // sedBuilderStack.addSegment(seg);
    }

    public void add(List<ExtSed> sedList) throws SedNoDataException,
        SedException, UnitsException 
    {
    ...
    public void remove(ExtSed sed) {
        ...
        // Segment seg;
        // try {
        // seg = ExtSed.flatten(sed0, sed0.getSegment(0).getSpectralAxisUnits(),
        // sed0.getSegment(0).getFluxAxisUnits()).getSegment(0);
        // nsed.addSegment(seg);
    ...
    // seg.setDataValues((double[])
    // nsed.getSegment(0).getDataValues(SEDMultiSegmentSpectrum.E_UTYPE),
    ...
    public static ExtSed createSedFromStack(SedStack stack)
            throws SedException, UnitsException {

        return createSedFromStack(stack, stack.getName());
    }

    public static ExtSed createSedFromStack(SedStack stack, String name)
            throws SedException, UnitsException {

        ExtSed sedBuilderStack = new ExtSed(name);

        for (ExtSed sed : stack.getSeds()) {
            ExtSed flattenedSed = ExtSed.flatten(sed, sed.getSegment(0)
                    .getSpectralAxisUnits(), sed.getSegment(0)
                    .getFluxAxisUnits());
    ...
    public void resetStack() {
        ...
        for (ExtSed sed : this.getOrigSeds()) {
            try {
                ...
                Logger.getLogger(SedStack.class.getName()).log(Level.SEVERE,
                        null, ex);
            } catch (SedException ex) {
                Logger.getLogger(SedStack.class.getName()).log(Level.SEVERE,
                        null, ex);
